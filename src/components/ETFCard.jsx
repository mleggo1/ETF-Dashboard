...
export const ETFCard = ({ etf, onChartClick }) => {
  ...
  return (
    <div className="group relative flex h-full flex-col gap-4 overflow-hidden rounded-2xl border border-slate-800/60 bg-slate-900/70 p-4 sm:p-5 shadow-[0_18px_45px_-25px_rgba(20,184,166,0.65)] transition hover:border-emerald-400/60 hover:shadow-[0_25px_65px_-25px_rgba(16,185,129,0.7)] min-h-[28rem]">
      ...
      <div className="relative flex items-start justify-between gap-3">
        <div>
          <div className="text-sm font-semibold uppercase tracking-[0.2em] text-emerald-200">{etf.symbol}</div>
      ...
      >
        <MiniLineChart timeframe={effectiveTf} data={data} />
        <div className="absolute inset-0 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity bg-slate-900/20 rounded-lg">
          <span className="text-xs uppercase tracking-[0.3em] text-emerald-300 font-medium">
            View Chart
          </span>
        </div>
      </div>
